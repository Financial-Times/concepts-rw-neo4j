...
func TestInvalidTypesThrowError(t *testing.T) {
	...
	for _, scenario := range scenarios {
		db.CypherBatch([]*neoutils.CypherQuery{{Statement: scenario.statementToWrite}})
...
func TestTransferConcordance(t *testing.T) {
	...
	db.CypherBatch([]*neoutils.CypherQuery{{Statement: statement}})
...
func TestProcessMembershipRoles(t *testing.T) {
	t.Skip("Test is still in progress")
	defer cleanDB(t)
