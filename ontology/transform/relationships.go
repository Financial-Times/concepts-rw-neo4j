...
func NAICSToRelationship(naics []ontology.NAICSIndustryClassification) ontology.Relationship {
	var connections []ontology.Connection
	for _, n := range naics {
		if n.UUID == "" {
			continue
...
func RelationshipsToNAICS(relations []ontology.Relationship) []ontology.NAICSIndustryClassification {
	var naics []ontology.NAICSIndustryClassification
	for _, rel := range relations {
		...
		for _, con := range rel.Connections {
			...
			naics = append(naics, ontology.NAICSIndustryClassification{
				UUID: con.UUID,
				Rank: rank,
			})
		}
...
func MembershipRolesToRelationship(roles []ontology.MembershipRole) ontology.Relationship {
	var connections []ontology.Connection
	for _, r := range roles {
		var (
...
func RelationshipsToMembershipRoles(relations []ontology.Relationship) []ontology.MembershipRole {
	var roles []ontology.MembershipRole
	for _, rel := range relations {
		...
		for _, con := range rel.Connections {
			...
			roles = append(roles, ontology.MembershipRole{
				RoleUUID:        con.UUID,
